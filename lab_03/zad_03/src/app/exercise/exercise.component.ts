...
export class ExerciseComponent {
  topThreeWords(questionsChatGpt: Question[]): WordCount[] {
    ...
    function convertToOutput(wordsCountArray: [string, number][]): WordCount[] {
      return wordsCountArray.reduce(
        (acc: WordCount[], curr: [string, number]) => {
          const record: WordCount = {
            word: curr[0] as string,
            count: curr[1] as number,
          };
          acc.push(record);
          return acc;
        },
